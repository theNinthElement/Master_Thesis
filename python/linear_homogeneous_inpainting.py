...
def derivative_xx(image, width, height):
...
def derivative_yy(image, width, height):
    #width, height = image.shape[0], image.shape[1]
    len = width * height
    dyy = np.ones(len)
    for i in range(width, len - width):
        dyy[i] = (image[1 + width] - 2 * image[i] + image[1 - width])
...
def linear_homogeneous_FSI():
    ...
    masked_width, masked_height =  finalImage.shape[0], finalImage.shape[1]

    image_1d = image.reshape(original_width*original_height)
    maskedImage_1d = finalImage.reshape(masked_width*masked_height)
    ...
    for n in range(0, num_of_steps):
        dxx = derivative_xx(maskedImage_1d, masked_width, masked_height)  # ndimage.correlate1d(presentImage, [1, -2, 1], axis=0)
        dyy = derivative_yy(maskedImage_1d, masked_width, masked_height)  # ndimage.correlate1d(currentImage, [1, -2, 1], axis=1)
        ...
        for i in range (0, masked_width*masked_height-2):
            if n == 0:
            ...
            presentImage[i] = alpha * (presentImage[i] + (timeStepSize * (dxx[i]+dyy[i]))) + (1 - alpha) * prevImage[i]
